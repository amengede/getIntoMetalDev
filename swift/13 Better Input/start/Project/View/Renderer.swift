...
class Renderer: NSObject, MTKViewDelegate {
    ...
    func drawLitObjects(renderEncoder: MTLRenderCommandEncoder?) {
        ...
        armForDrawing(renderEncoder: renderEncoder, material: artyMaterial)
        for cube in scene.cubes {
            draw(renderEncoder: renderEncoder, modelTransform: &(cube.model!), meshType: 0)
        }
        
        armForDrawing(renderEncoder: renderEncoder, material: woodMaterial)
        for ground in scene.groundTiles {
            draw(renderEncoder: renderEncoder, modelTransform: &(ground.model!), meshType: 1)
        }
        
        armForDrawing(renderEncoder: renderEncoder, material: mouseMaterial)
    ...
    func drawUnlitObjects(renderEncoder: MTLRenderCommandEncoder?) {
        ...
        armForDrawing(renderEncoder: renderEncoder, material: lightMaterial)
    ...
    func armForDrawing(renderEncoder: MTLRenderCommandEncoder?, material: Material) {
