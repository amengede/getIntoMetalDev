...
class Renderer: NSObject, MTKViewDelegate{
    ...
    init(_ parent: MetalView){
        ...
        camera.viewMatrix = Matrix44.create_lookat(eye: [0, 2, -2], target: [0,0,0], up: simd.normalize([0, 1, 1]))
        camera.projectionMatrix = Matrix44.create_perspective_projection(fovy: 90, aspect: 1, near: 0.1, far: 100)
        camera.viewProj = camera.projectionMatrix * camera.viewMatrix
    }
    ...
    func draw(in view: MTKView) {
        ...
        camera.modelMatrix = Matrix44.create_from_rotation(eulers: [0,t,0])
        camera.modelMatrix = Matrix44.create_from_translation(translation: [0, 0.3, 0]) * camera.modelMatrix
        ...
        for submesh in self.metalMesh.submeshes {
